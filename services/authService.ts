...
// Authentication is now handled by keyAuthService.ts with Supabase
// This file is kept only for the hashPin utility function used in settings
...
// DEPRECATED: SQLite-based authentication is no longer used
// Authentication is now handled by Supabase (see keyAuthService.ts)
export async function initializeAuthLayer() {
  // No longer needed - authentication is handled by Supabase
  console.log('[authService] Deprecated - authentication now handled by Supabase');
}
...
export async function enableBiometrics(userId: string) {
  // No longer using SQLite - just store in SecureStore
  await Promise.all([
    SecureStore.setItemAsync(BIOMETRIC_PREFERENCE_KEY, '1', secureStoreOptions()),
    SecureStore.setItemAsync(BIOMETRIC_USER_KEY, userId, secureStoreOptions()),
  ]);
}

export async function disableBiometrics(userId: string) {
  // No longer using SQLite - just clear from SecureStore
  await Promise.all([
...
export async function getBiometricUserId(): Promise<string | null> {
  ...
  // Return the value as string (Supabase UUID)
  return value;
}
...
export async function recreateSessionForUser(userId: string): Promise<any | null> {
  // This function now delegates to keyAuthService for Supabase users
  const { recreateSessionForUser: recreateSupabaseSession } = require('./keyAuthService');
  return await recreateSupabaseSession(userId);
}
